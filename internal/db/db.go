...
func NewDbManager(config *config.ServerConfig) *DbManager {
	dsn := fmt.Sprintf("host=%s user=%s password=%s dbname=%s port=%s sslmode=disable TimeZone=UTC", config.DBHost, config.DBUser, config.DBPass, config.DBName, config.DBPort)
