...
func registerPasswordHandlers(r *mux.Router) {
	subRouter := r.PathPrefix(RoutePassword).Subrouter()
	for _, handler := range passwordHandlers {
		contextLogger.Info(MessageRouteRegistered, LogKeyRoute, RoutePassword, LogKeySubroute, handler.Route, LogKeyMethod, handler.Method)
...
func addPassword(w http.ResponseWriter, r *http.Request) {
	...
	if result.RowsAffected > 0 {
		writeErrorResponse(r.Context(), w, http.StatusBadRequest, ErrorDuplicatePassword)
		return
	}

	value, err := hex.DecodeString(addPasswordRequest.Value)
	if err != nil {
		writeErrorResponse(r.Context(), w, http.StatusInternalServerError, ErrorInternalServer, "Could not transform password value to byte array")
		return
	}

	iv, err := hex.DecodeString(addPasswordRequest.IV)
	if err != nil {
		writeErrorResponse(r.Context(), w, http.StatusInternalServerError, ErrorInternalServer, "Could not transform password IV to byte array")
		return
	}

	authTag, err := hex.DecodeString(addPasswordRequest.AuthTag)
	if err != nil {
		writeErrorResponse(r.Context(), w, http.StatusInternalServerError, ErrorInternalServer, "Could not transform password auth tag to byte array")
		return
	}

	salt, err := hex.DecodeString(addPasswordRequest.Salt)
	if err != nil {
		writeErrorResponse(r.Context(), w, http.StatusInternalServerError, ErrorInternalServer, "Could not transform keygen salt to byte array")
		return
	}
	...
	if result.RowsAffected == 0 {
		writeErrorResponse(r.Context(), w, http.StatusInternalServerError, ErrorCreationFailed, "Could not create password")
		return
	}
...
func deletePassword(w http.ResponseWriter, r *http.Request) {
	...
	if result.RowsAffected == 0 {
		writeErrorResponse(r.Context(), w, http.StatusInternalServerError, ErrorInternalServer, "Failed to delete password")
		return
	}
